...
class SitemapItem{
    ...
    public function __construct($data=array()) {
        if(func_num_args() && is_array($data)){
            ...
            if(isset($data['changefreq'])) $this->changefreq($data['changefreq']);
        }
    }
...
class Sitemap
{
    ...
    public $generator = 'ELI 2012/v2 - Elixom APi by Shane Edwards';
    public $xml_version = '1.0';
    public $xml_encoding = 'UTF-8';
    public $xml_stylesheet = '';
    public $nameSpace = 'http://www.sitemaps.org/schemas/sitemap/0.9';
     ...
     * @return void
     */
    public function add($url,$lastmod=null,$changefreq='none',$priority=0)
	{
		$row['href'] = $url;
        ...
        $el = new SitemapItem($row);
        $this->items[] = $el;
        return $el;
	}
    
    public function inject($url,$lastmod=null,$changefreq='none',$priority=0)
	{
	   //DEPRECATED (from ELI_sitemap)
        return $this->add($url,$lastmod,$changefreq,$priority);
	}
    public function __toString(){
    ...
    public static function submit($url, $services=array('google','bing'))
    {
        $sm = urlencode($url);
        $log = $urls = array();
